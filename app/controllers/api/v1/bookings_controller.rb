class Api::V1::BookingsController < Api::V1::ApiController
  ...
  def notification_worker
    if @booking.present?
      ...
      two_hour_worker = NotificationWorker.perform_in((@booking.end_time.last - (@booking.end_time.last - @booking.start_time.first)), @booking.send_to_id, "#{((@booking.end_time.last - @booking.start_time.first) / 60).to_i}", @booking.send_by.id, @booking.send_by.full_name, "Post Reminder Initial", booking_sender_id: @booking.send_by_id)
